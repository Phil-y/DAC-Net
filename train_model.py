...
def logger_config(log_path):
    ...
    # print(loggerr) #<RootLogger root (WARNING)>
    loggerr.setLevel(level=logging.INFO)
    handler = logging.FileHandler(log_path, encoding='UTF-8')
    handler.setLevel(logging.INFO)
    formatter = logging.Formatter('%(message)s')
    # print(formatter) #<logging.Formatter object at 0x00000205678C39D0>
    handler.setFormatter(formatter)
    console = logging.StreamHandler()
    # print(console)#<StreamHandler <stderr> (NOTSET)>
...
def main_loop(batch_size=config.batch_size, model_type=config.model_name, tensorboard=True):
    ...
    # print(val_tf) #<Load_Dataset.ValGenerator object at 0x0000026A6A2D4B80>
